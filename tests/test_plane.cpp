...
void testLuggageValues()
{
    ...
    std::shared_ptr<Economy> econom = std::make_shared<Economy>(10, 24);
    std::shared_ptr<Business> business = std::make_shared<Business>(24, 40);
    std::shared_ptr<FirstClass> firstClass = std::make_shared<FirstClass>(60, 100);
...
void testAddLuggageLogic()
{
    ...
    std::shared_ptr<FlightAttendant> flightAttendant = std::make_shared<FlightAttendant>();
    ...
    plane2.addPassenger(std::make_shared<Economy>(10, 24)); //34
    plane2.addPassenger(std::make_shared<Economy>(10, 24)); //68
    assert(plane2.addPassenger(std::make_shared<Economy>(10, 24)) == true); //102 > 100 -> пассажир добавится, но без багажа
    ...
    assert(plane2.addPassenger(std::make_shared<Business>(24, 27)) == true); //51>50 -> переносим в эконом
    ...
    assert(plane2.addPassenger(std::make_shared<Business>(20, 0)) == true);
    assert(plane2.getCurrentLuggageWeightBusiness() == 44); //теперь в бизнесе будет 44 кг
    assert(plane2.addPassenger(std::make_shared<Business>(7, 1)) == false); //добавим бизнес пассажира, чтобы не поместилась ручная кладь 44+7 > 50
    assert(plane2.getCurrentLuggageWeightBusiness() == 44); // в бизнесе все также будет 44 кг

    //теперь сажаем бизнес класс
    assert(plane2.addPassenger(std::make_shared<FirstClass>(0, 50)) == true); //полностью забили допустимый вес
    assert(plane2.addPassenger(std::make_shared<FirstClass>(1, 0)) == false); //теперь не можем посадить даже легкого пассажира
}
